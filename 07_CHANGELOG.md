...
## 2026-06-20 | frontend+backend | Magic-link sign-in fixed (was bouncing back to the sign-in page)

**Problem:** clicking the emailed magic link returned the user to the portal sign-in form.
**Root cause (multi-layer):** the intended cross-domain auth (portal on pariscomedy.com, API on
api.pariscomedy.com; session token in localStorage sent as `Authorization: Bearer`) was broken at
every layer, so `/api/me` always returned `signed_in:false`:
1. CORS `allow_headers` omitted `Authorization` → browser preflight blocked the Bearer request.
2. `assets/auth-helpers.js` `_readBearerToken()` read `pc_token`/`token`/`booker_token`, but
   `auth/verify.html` stores the session as `pc_session_token` → no header was ever sent.
3. `main.py` `_current_user()` only read the `pc_auth` cookie; it never inspected the Bearer header
   (and the cookie path was dead too — `verify.html`'s fetch lacked `credentials:'include'`).
**Fix:** (1) CORS allow `Authorization`; (2) `_readBearerToken` reads `pc_session_token`;
(3) `_current_user` accepts `Authorization: Bearer <token>`, cookie kept as same-domain fallback.
Verified by reproduction (Bearer signed_in false→true) + full headless-Chrome flow (valid session →
portal renders, sign-in hidden). Deployed `auth-helpers.js` (commit `e1d32ce`); backend restarted.

## 2026-06-20 | frontend+data | Homepage re-wired LIVE (root-cause fix for stale "tonight" + dead links)

**Problem (regression):** the homepage showed no tonight's shows and carried dead ticket
links. Root cause: `index.html` rendered a hardcoded static `SHOWS_DATA`/`TOP_SHOWS`/`SHOWS`
snapshot "generated by refresh_pariscomedy.py" — but that generator no longer existed and
no cron ran it, so the baked dates (≤2026-06-17) rotted → `TONIGHT_SHOWS`/`WEEK_SHOWS` empty.
Separately, 3 of 20 active links were dead billetreduc events; `site_guard` R4 missed them
because billetreduc injects a JSON-LD `startDate == page-render time` that `extract_latest_date`
read as a live future date.

**Fix (eliminates the class, not a re-patch):**
- `index.html`: removed all 3 static arrays (−43 KB). The page now **fetches `/api/listings`
  live** and expands recurring shows into dated instances in-browser (`loadLiveShows`,
  `expandListings`, `recomputeWindows`). No static array, no generator cron → cannot go stale.
  Verified via headless Chrome render: "Tonight" populates from the live DB.
- DB: archived the 3 dead billetreduc shows (`bootleg-bar-comedy-club`, `le-lieu-de-rire`,
  `well-com-le-plateau`) — `status='archived', public_visible=0` + reason. Active set 20→17,
  all link-verified currently selling (20-link adversarial verification: 17 selling / 3 dead).
- `scripts/site_guard.py`: **R0** fail if 0 active+visible listings; **R4a** hard-fail a
  billetreduc page showing "n'est plus disponible à la vente / aucune date prévue" regardless
  of placeholder startDate; **R10** fail if `index.html` is not wired to live `/api/listings`
  or re-introduces a static dated array. Enforced by the pre-push hook.
- `deploy_fix.sh`: narrowed the injected-markup scan to true XSS vectors (`<script src>`,
  `javascript:`, `on*=`, `document.cookie`) so legitimate `fetch()`/`localStorage` frontend
  code is not falsely blocked.

**DO NOT** reintroduce a static `SHOWS_DATA` array in `index.html` — R10 will block the push.
