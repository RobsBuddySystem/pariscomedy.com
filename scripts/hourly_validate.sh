...
: >"$LOG"; : >"$ERR"   # truncate at start of each run
{
  ...
  if git diff --quiet -- data/shows_generated.json data/review_queue.json data/discovered_shows.json data/scrape_conflicts.json data/organizers.json index.html shows.html; then
    echo "no-op (no diff)"
    exit 0
  fi

  git add data/shows_generated.json data/review_queue.json data/discovered_shows.json data/scrape_conflicts.json data/organizers.json index.html shows.html
