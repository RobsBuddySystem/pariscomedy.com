...
PUBLIC_PAGES = [
    ...
    ("europe.html",      "/europe.html",      "daily",   "0.9"),
...
def main() -> None:
    ...
    # Europe hub — static per-event pages generated by build_europe_pages.py.
    # These live at europe/<city-slug>/<event-slug>.html and are discovered
    # directly on disk (no separate manifest needed — build_europe_pages.py
    # already only writes a page per real event in upcoming_events.json).
    europe_dir = REPO / "europe"
    if europe_dir.is_dir():
        for page in sorted(europe_dir.rglob("*.html")):
            if page.name == "go.html":
                continue  # redirect utility page, not indexable content
            rel = page.relative_to(REPO).as_posix()
            loc = "/" + rel
            entries.append(url_entry(loc, git_lastmod(rel), "daily", "0.6"))
